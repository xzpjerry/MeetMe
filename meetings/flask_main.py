...
def get_range():
    ...
    if duration:
        Set_range = model.eventrange(
            flask.session['begin_date'], flask.session['end_date'], duration)
    else:
        Set_range = model.eventrange(
...
def get_result():
    ...
    for collection in DB.all_user_collection():
        if "MeetingUser" in collection:
            ...
            if user_has_selected_or_default == None or collection in flask.session["Checked_users"]:
                user_flash_result += (collection + " ")
                DB.set_collection(collection)
                for event in DB.get_all():
                    test_record = model.record(None, None, event)
                    test_event = model.calendar_event(
                        test_record.start, test_record.end)
    ...
    for key in keys:
        for chunk in busy_list[key]['busy']:
            ...
            if tmp_event.compare_to(Set_range) == model.event_compare_result.within:
                DB.insert(model.record(
                    chunk['start'], chunk['end'], None, None))
...
def _update_duration():
    ...
    if result.isdigit():
        ...
        app.logger.debug(
            "Got a updating duration request for %s mins." % result)
    return flask.jsonify(success=True)
...
def _updateSelectedUsers():
    ...
    print(flask.session["Checked_users"])
    return flask.jsonify(success=True)
